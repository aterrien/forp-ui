...
{f.DOMElementWrapper.call(this);this.element=document.createElement("div");var percent=0,text,displayedValue;displayedValue=f.roundDiv(value,(divider?divider:1));if(value>max){text="reached "+displayedValue.toFixed(3)+(unit?unit:'');}else if(value<0){text="won "+Math.abs(displayedValue).toFixed(3)+(unit?unit:'');}else{text=displayedValue.toFixed(3)+(unit?unit:'');percent=f.round(value*100/max);}
...
{this.stack=conf.stack;this.id=conf.id;this.class=conf.class;this.function=conf.function;this.refs=[];this.entries=[];this.calls=1;this.duration=conf.duration;this.memory=conf.memory;this.setEntry=function(filelineno,entry)
