...
if((i==this.stack.length)&&(this.stack.length<this.size)){this.insert(entry,i);}}else{this.insert(entry,0);}};}};})(forp);(function(f){"use strict";forp.DOMElementWrapper=function(element)
...
{return new f.DOMElementWrapper(this.elements[i]);};};})(forp);(function(f){"use strict";forp.Controller=function(stack)
...
{this.attr("style","").empty().class(this.viewMode);this.nav=null;this.mainpanel=null;callback();return this;};document.body.insertBefore(this.element,document.body.firstChild);};forp.Nav=function()
{var self=this;f.DOMElementWrapper.call(this);this.element=document.createElement("nav");};forp.Panel=function(id)
{var self=this;f.DOMElementWrapper.call(this);this.element=document.createElement("div");this.class(id+" panel");this.id=id;};forp.MainPanel=function(layout)
...
return this.console;};this.open=function(){this.layout.size();return this;};this.close=function(){self.css("height: 0px",function(){});return this;};};forp.ToggleBar=function()
{var self=this;f.DOMElementWrapper.call(this);this.element=document.createElement("div");this.class("toggleBar");this.clear=function(){this.find("div.toggleOn").each(function(o){o.class("toggleOff");o.attr("data-state","off")});return this;};};forp.ToggleButton=function(label,on,off,triggerOn)
{var self=this,click=null;f.DOMElementWrapper.call(this);this.element=document.createElement("div");this.text(label).class("toggleOff").bind('click',click=function(e){if(self.getAttr("data-state")=="on"){off&&(off(e)!==false)&&self.class("toggleOff").attr("data-state","off");}else{self.parent&&self.parent.clear&&self.parent.clear();on&&(on(e)!==false)&&self.class("toggleOn").attr("data-state","on");}});triggerOn&&this.trigger("click");};forp.Sidebar=function(parent)
{var self=this;f.Panel.call(this,"sidebar");this.addClass("w1of3");this.parent=parent;};forp.Console=function(parent)
...
return this;};};forp.Table=function(headers)
...
this.line=function(cols){return(new f.Line(cols)).appendTo(this);}};forp.Line=function(cols)
{f.DOMElementWrapper.call(this);this.element=document.createElement("tr");for(var i in cols){if(typeof cols[i]==="object"){f.create("td",this,"","numeric w100").append(cols[i]);}else if(isNaN(cols[i])){f.create("td",this,cols[i]);}else{f.create("td",this,cols[i],"numeric w100");}}};forp.Tree=function(stack)
...
return ul;};this.append(this.treeList(stack[0],true));};forp.Backtrace=function(i,stack)
...
f.roundDiv(entry.bytes,1024).toFixed(3)+"Kb"));};var child=i;while(i!=null){this.prependItem(stack[i],child==i);i=stack[i].parent;if(i!=null){this.prepend(f.create("div").class("arrow").text("&#x25BC;"));}}};forp.LineEventListenerBacktrace=function(i,context)
{this.target=null;this.init=function()
{this.target.attr("data-ref",i).bind("click",function(e){e.preventDefault();e.stopPropagation();var line=f.wrap(this);context.getConsole().getSidebar().empty().append(new f.Backtrace(line.getAttr("data-ref"),context.getStack().stack)).scrollBottom();});}};forp.Gauge=function(value,max,divider,unit)
...
this.addClass("gauge").append(f.create("div").class("text").text(text)).append(f.create("div").addClass("bar").attr("style","width: "+percent.toFixed(0)+"%"));};forp.TagRandColor={i:0,pocket:["#f95","#f59","#59f","#5e9","#9e6","#95f","#e55","#fe6","#f6f","#5e5","#5ef","#55f"],tagsColor:{},provideFor:function(name)
...
{return f.create("a").class("tag").attr('style','background: '+this.provideFor(name)).text(name).bind("click",function(){});}};})(forp);(function(f){"use strict";forp.Stack=function(stack)
