...
{return new f.DOMElementWrapper(this.elements[i]);};};})(forp);(function(f){"use strict";f.DOMElementWrapperCollection.prototype.forp=function(conf){this.each(function(el){conf.parent=el.element;(new f.Controller(conf)).setStack(conf.stack).run();});},f.Controller=function(conf)
...
{this.target.attr("data-ref",i).bind("click",function(e){e.preventDefault();e.stopPropagation();var line=f.wrap(this);context.getConsole().getSidebar().empty().append(new f.Backtrace(line.getAttr("data-ref"),context.getStack().stack)).scrollBottom();});}};f.Gauge=function(value,max,divider,unit)
