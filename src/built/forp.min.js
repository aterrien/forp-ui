...
(function($){"use strict";$.fn.forp=function(options){options=$.extend({stack:[],mode:"embedded"},options);(new forp.Controller({parent:this[0],mode:options.mode})).setStack(options.stack).run();};})(jMicro);(function(f,$){"use strict";$.fn.scrollBottom=function(){return this.each(function(){this.scrollTop=$(this.firstChild).height();});};$.table=function(headers){var $table=$('<table>');if(headers){var $header=$("<tr>");for(var i in headers){$("<th>").text(headers[i]).appendTo($header);}
...
{forp.Decorator.call(this,$("<div>"));this.$.class(id+" panel");this.id=id;};})(forp,jMicro);(function(f,$){"use strict";forp.Controller=function(conf)
...
{if(!this.memoryHistogram){this.memoryHistogram=(new forp.Histogram({xaxis:{min:0},yaxis:{length:50,min:0,max:self.getStack().getTopMemory()[0].bytes},val:function(i){return this.datas[i].bytes;},mousemove:function(e){var x=Math.floor((e.offsetX*this.$.width())/this.$.width()),y=this.conf.yaxis.length
...
{var self=forp.getController(),target=$(this);if(target.attr("data-details")==1){target.nextSibling().remove();target.attr("data-details",0);return;}
...
this.close=function(){this.$.removeClass("w1of3");return this;}};})(forp,jMicro);(function(forp,$){forp.Backtrace=function(i,stack)
