...
return result;},parent:function(){return new $(this[0].parentNode);}};w.jMicro.fn.construct.prototype=w.jMicro.fn;w.jMicro.extend=w.jMicro.fn.extend=function(){var len=arguments.length,target,start=0;var deep=false;if(len==1){for(var entry in arguments[0]){var obj=arguments[0][entry];if(typeof obj!='function'&&obj instanceof Object){w.jMicro[entry]=w.jMicro.extend(w.jMicro.fn[entry],obj);}else{w.jMicro[entry]=w.jMicro.fn[entry]=obj;}}
...
(function(forp,$){"use strict";$.fn.forp=function(options){options=$.extend({stack:[],mode:"embedded"},options);(new forp.Controller({parent:this[0],mode:options.mode})).setStack(options.stack).run();};})(forp,jMicro);(function(forp,$){"use strict";$.fn.scrollBottom=function(){return this.each(function(){this.scrollTop=$(this.firstChild).height();});};$.table=function(headers,reorder){var $table=$('<table>');if(headers){var $header=$("<tr>");for(var i in headers){if(reorder&&~reorder.indexOf(headers[i])){$("<th>").text(headers[i]).addClass("reorder").on("click",$.fn.tableOrder).appendTo($header);}else{$("<th>").text(headers[i]).appendTo($header);}}
$header.appendTo($table);}
return $table;};$.fn.tableOrder=function(e){$(e.srcElement).data("order",$(e.srcElement).data("order")?0:1);var $tbl=$(e.srcElement).parent().parent(),index=e.srcElement.cellIndex,trs=[],order=$(e.srcElement).data("order");for(var i=1;i<$tbl[0].childNodes.length;i++){trs.push($tbl[0].childNodes[i]);}
trs.sort(function(a,b){var aval=a.childNodes[index].innerHTML.replace(/<.*?>/g,''),bval=b.childNodes[index].innerHTML.replace(/<.*?>/g,''),regex=/^[\d\.]+$/;if(regex.test(aval)&&regex.test(bval)){return order?(parseFloat(aval)-parseFloat(bval)):(parseFloat(bval)-parseFloat(aval));}else if(aval<bval){return order?-1:1;}else if(aval>bval){return order?1:-1;}
return 0;});for(var i=0;i<trs.length;i++){$tbl.append(trs[i]);}};$.fn.line=function(cols){var $tr=$('<tr>');for(var i in cols){if(typeof cols[i]==="object"){$tr.append($("<td>").append(cols[i]));}else if(isNaN(cols[i])){$tr.append($("<td>").html(cols[i]));}else{$tr.append($("<td>").addClass("numeric w100").html(cols[i]));}}
...
self.getConsole().show($table);},'close':function(){forp.getController().getLayout().reduce();}}};})(forp,jMicro);(function(forp,$){forp.plugins.files={'nav':{'label':'files','display':function(){return forp.getController().getStack().includesCount>0;},'enabled':false,'open':function(e){var controller=forp.getController(),datas=controller.getStack().getIncludes();var $table=$.table(["file","calls from"],["file","calls from"]);for(var i in datas){$table.line([i,$.Gauge(datas[i].calls,controller.getStack().stack.length)]);}
